"""
Entry script of using OAT to RL-tune LLM agents on static datasets.
"""
...
from oat.utils.math_grader import (answer_tag_reward_fn, boxed_reward_fn,
...
def apply_llama_instruct_template(question: str) -> str:
    question = (
        question
        + " Please reason step by step, and put your final answer within \\boxed{}.\n"
    )
...
TEMPLATE_FACTORY = {
    ...
    "llama3": apply_llama_instruct_template,
}
...
class ZeroMathArgs(PPOArgs):
    ...
    prompt_template: Literal[
        "qwen_math", "no", "r1", "r1_distill_qwen", "qwen3", "llama3"
    ] = field(default="qwen_math")
    apply_default_chat_template: bool = False
    # Evaluation benchmarks used.
    test_split: str = "all"  # Use "aime,math" to only evaluate on selected benchmarks.
    # Verifier.
    verifier_version: Literal["fast", "math_verify"] = field(default="math_verify")
    ...
    clip_high: float = 0.2
    # Importance sampling.
    use_seq_is: bool = False
    use_token_is: bool = False
    ppo_use_seq_is: bool = False
    seq_is_clip_max: float = torch.inf
    token_is_clip_max: float = torch.inf
    use_gspo: bool = (
...
class ZeroMathLearner(PPOLearner):
    ...
    def prepare_data(self, strategy, tokenizer):
        ...
        self.prompts_dataset = PromptDataset(
            ...
            apply_chat_template=args.apply_default_chat_template,
            get_reference=True,
        )
    ...
    def learning_step(self, trajectory):
        ...
        for _ in range(args.num_ppo_epochs):
            ...
            for b_st in range(0, len(input_ids), args.train_batch_size_per_device):
                ...
                elif args.use_seq_is:
                    # Simple off-policy PG with (truncated) sequence-level importance sampling
                    token_logprobs_diff = (
                        new_logps.detach() - mb_actor_logps
                    ) * mb_response_masks
                    seq_is = torch.exp(token_logprobs_diff.sum(dim=-1, keepdim=True))
                    seq_is_truncated = seq_is.clamp(max=args.seq_is_clip_max)
                    pg_loss_max = -seq_is_truncated * mb_advantage * new_logps
                    stats["seq_is_pg_clipfrac"].append(
                        torch.gt(seq_is, args.seq_is_clip_max).float().mean()
                    )
                elif args.use_token_is:
                    # (Truncated) Token-level IS approximation to reduce variance (biased)
                    token_logprobs_diff = (
                        new_logps.detach() - mb_actor_logps
                    ) * mb_response_masks
                    token_is = torch.exp(token_logprobs_diff)
                    token_is_truncated = token_is.clamp(max=args.token_is_clip_max)
                    pg_loss_max = -token_is_truncated * mb_advantage * new_logps
                    stats["token_is_pg_clipfrac"].append(
                        torch.gt(token_is, args.token_is_clip_max).float().mean()
                    )
                else:
                    # Proximal policy optimization
                    if args.use_gspo:
                        # Use sequence-level IS instead of token-level for unbiasedness;
                        # but GSPO took geometric mean on the IS, which is biased again.
                        logprobs_diff = (
                            new_logps.detach() - mb_logps
                        ) * mb_response_masks
                        seq_is_with_geometric_mean = torch.exp(
                            logprobs_diff.sum(dim=-1, keepdim=True)
                            / mb_response_masks.sum(dim=-1, keepdim=True)
                        )
                        new_probs = torch.exp(new_logps)
                        ratio = (
                            seq_is_with_geometric_mean * new_probs / new_probs.detach()
                        )
                        pg_losses = -mb_advantage * ratio
                        pg_losses2 = -mb_advantage * torch.clamp(
                            ratio, 1.0 - 3e-4, 1.0 + 4e-4
                        )
                        ...
                        pg_losses = -mb_advantage * ratio
                        pg_losses2 = -mb_advantage * torch.clamp(
                            ratio, 1.0 - args.clip_low, 1.0 + args.clip_high
                        )
                        pg_loss_max = torch.max(pg_losses, pg_losses2)
                        if args.ppo_use_seq_is:
                            token_logprobs_diff = (
                                mb_logps - mb_actor_logps
                            ) * mb_response_masks
                            seq_is = torch.exp(
                                token_logprobs_diff.sum(dim=-1, keepdim=True)
                            )
                            clip_mask = (seq_is <= args.seq_is_clip_max).float()
                            pg_loss_max *= clip_mask * seq_is
                            stats["seq_is_pg_clipfrac"].append(
                                torch.gt(seq_is, args.seq_is_clip_max).float().mean()
                            )
                ...
                kl_v2_sum += masked_sum(token_diff**2, mb_response_masks, axis=1).sum()
                ...
                with torch.no_grad():
                    if args.reinforce_update or args.use_seq_is or args.use_token_is:
                        pass
                    else:
        ...
        if args.reinforce_update:
            pass
        elif args.use_seq_is or args.ppo_use_seq_is:
            stats["seq_is_pg_clipfrac"] = torch.tensor(
                stats["seq_is_pg_clipfrac"]
            ).mean()
        elif args.use_token_is:
            stats["token_is_pg_clipfrac"] = torch.tensor(stats["token_is_pg_clipfrac"])
        else:
